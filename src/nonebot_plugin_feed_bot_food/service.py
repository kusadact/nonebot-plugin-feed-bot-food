...
from .models import BotState, DailyStats, FeedEvent, empty_state, quantize_weight
...
class FeedService:
    def __init__(
        ...
        gain_generator: RandomGainGenerator,
    ) -> None:
        self.config = config
        self.store = store
        self.gain_generator = gain_generator
    ...
    async def _feed(
        ...
        async with self.store.lock:
            ...
            gain = quantize_weight(self.gain_generator.generate())
            event = FeedEvent(
                user_id=user_id,
                food=food,
                gain=gain,
