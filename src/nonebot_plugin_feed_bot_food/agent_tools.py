    ...
    def build_feed_bot_food_tools(ctx: AgentToolContext) -> AgentToolBundle | None:
        ...
        return AgentToolBundle(
            ...
            instructions=[
                ...
                "- feed_bot_food 返回 success、category_limited、request_limited 或 invalid_food 时，依据返回的 message 和字段向用户回复；不要重复调用工具。",
                "- feed_bot_food 返回 ignored 或 llm_error 时静默结束本次处理，不要向用户发送消息；这两种状态表示分类/请求失败。",
